...
import { applyDailyRollover, clearAllDone, formatLocalDate, millisecondsUntilNextLocalMidnight } from "./state.js";
...
const oldState = {
  rooms: {
    [ROOM_IDS.KITCHEN]: {
      ...
      tasks: DEFAULT_ROOMS[ROOM_IDS.KITCHEN].tasks.map((task, index) => ({ ...task, done: index === 0 })),
    },
  },
  routines: {
    daily: [{ id: "d-test", title: "Daily", done: true }],
    weekly: [{ id: "w-test", title: "Weekly", done: true }],
    lastDailyReset: formatLocalDate(yesterday),
    lastWeeklyReset: "2026-W28",
  },
  progress: { daily: [], weekly: [] },
  meta: { schemaVersion: 3, lastResetDate: formatLocalDate(yesterday) },
};
